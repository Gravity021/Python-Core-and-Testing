import pygame, os
from Framework import image_handling, json_handler

def load_tilesets(dir, colours, colourkey=None):
    tilesets = {}
    for file in os.listdir(dir):
        if file.endswith(".png"):
            path = file.split(".")[0]
            tile_order = json_handler.load(dir + "/" + path + ".json")
            tilesets[path] = Tileset(dir + "/" + file, colours, tile_order, colourkey)
    return tilesets

class Tileset:
    def __init__(self, img_path, colours, order, colourkey=None):
        self.img_path = img_path
        self.tileset_img = pygame.image.load(self.img_path).convert()

        if colourkey != None:
            self.tileset_img.set_colorkey(colourkey)

        images = image_handling.split_tiles(self.tileset_img, colours)
        self.images = {}
        for i in range(len(images)):
            self.images[order[i]] = images[i]